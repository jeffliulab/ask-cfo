...
### Git 流程
- **版本封版后直推 main，无需 PR，无需 ask**：`git tag vX.Y.Z` 之后 agent 应主动 `git push origin main && git push origin vX.Y.Z`
- 开发中（mid-version）的 commit 默认也直推 main（pre-1.0 单人开发；agent-rules/workflows/git.md 的"Push 前必须问"在 ask-cfo 项目级被放宽）
- 例外：要做大重构 / 引入破坏性变更（删 alembic migration、改 SQLite schema 等）时，仍要先 ack 再 push
- Conventional Commits 格式（`feat(v0.X.0):` / `chore:` / `fix:` / `docs:`）
- push 后回报 commit / tag URL 给用户
